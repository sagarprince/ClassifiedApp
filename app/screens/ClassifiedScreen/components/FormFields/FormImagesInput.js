...
const FormImagesInput = (props) => {
    ...
    const onChanges = useCallback((_images) => {
    ...
    const showImagePicker = useCallback(() => {
        ...
        ImagePicker.showImagePicker(options, (response) => {
            if (response.error) {
                ...
                !response.didCancel && setImages(images => {
                    const { uri } = response;
                    const selectedImage = {
                    ...
                    const allImages = [...images, selectedImage];
            ...
            <View style={[styles.grid, { ...style }]}>
                    ...
                    return (
                        <View style={styles.imageBox} key={item.id}>
                            ...
                            <TouchableOpacity style={styles.removeImageBtn} onPress={() => removeImage(item.id)}>
                                <Icon type="Entypo" name="cross" style={styles.removeImageIcon}></Icon>
                            </TouchableOpacity>
                        </View>
                    );
                })}
                {images.length !== limit &&
                    <TouchableOpacity style={styles.addBtn} onPress={showImagePicker}>
                        <Icon type="Entypo" name="plus" style={styles.addBtnIcon}></Icon>
                    </TouchableOpacity>}
            </View >
