...
const AddEditProductForm = ({categoryId, productInfo, type, mode}) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    price: Yup.mixed()
      ...
      .test('isNumber', 'Please enter a valid product price.', value => {
        const pattern = /^\d+$/;
  ...
  const showToast = (text, toastType) => {
    Toast.show({text, type: toastType, textStyle: {fontSize: 15}});
  };
  ...
  useEffect(() => {
    return () => setPlaces('productform', {entities: []});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
    ...
    <Formik
      ...
      {({
        ...
        <View>
          ...
          <View style={styles.marginTop}>
            <FormInput
              label={type === 'sell' ? 'Selling Price' : 'Renting Price'}
          ...
          <View style={[styles.marginTop, styles.productImagesWrap]}>
            <Text style={styles.productImagesHeading}>
          ...
          <View style={styles.actions}>
            <Button
              ...
              disabled={productCRUD.isLoading}>
              {productCRUD.isLoading && (
                <ActivityIndicator
                  size="small"
                  color="#fff"
